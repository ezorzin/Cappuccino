...
/// @author   Erik ZORZIN
/// @date     22JUL2020
/// @brief    First small and tasty Neutrino application of a Cellular Automata For Epidemics.

// OPENGL:
#define GUI_SIZE_X    800                                                                           // Window x-size [px].
#define GUI_SIZE_Y    600                                                                           // Window y-size [px].
#define GUI_NAME      "CafeCodes - Cappuccino"                                                      // Window name.

// OPENCL:
#define QUEUE_NUM     1                                                                             // Number of OpenCL queues [#].
#define KERNEL_NUM    1                                                                             // Number of OpenCL kernel [#].
#define KERNEL_FILE   "tile_3_kernel.cl"                                                            // OpenCL kernel.
...
#define SHADER_VERT   "tile_3.vert"                                                                 // OpenGL vertex shader.
#define SHADER_GEOM   "tile_3.geom"                                                                 // OpenGL geometry shader.
#define SHADER_FRAG   "tile_3.frag"                                                                 // OpenGL fragment shader.
...
int main ()
{
  ...
  size_t                   nodes_x            = 101;                                                // Number of nodes in "X" direction [#].
  size_t                   nodes_y            = 101;                                                // Number of nodes in "Y" direction [#].
  ...
  float4G*                 color_1            = new float4G ();                                     // OpenGL float4G.
  float4G*                 color_2            = new float4G ();                                     // OpenGL float4G.
  float4G*                 color_3            = new float4G ();                                     // OpenGL float4G.
  float                    A                  = 0.7;                                                // Amplitude of gaussian.
  float                    ofs                = 0.3;                                                // Offset of gaussian.
  ...
  color_1->init (nodes);                                                                            // Initializing OpenGL color_1 array...
  color_2->init (nodes);                                                                            // Initializing OpenGL color_2 array...
  color_3->init (nodes);                                                                            // Initializing OpenGL color_3 array...
  t->init (nodes);                                                                                  // Initializing time...

  position->name = "tile_center";                                                                   // Setting variable name in OpenGL shader...
  color_1->name  = "tile_color_1";                                                                  // Setting variable name in OpenGL shader...
  color_2->name  = "tile_color_2";                                                                  // Setting variable name in OpenGL shader...
  color_3->name  = "tile_color_3";                                                                  // Setting variable name in OpenGL shader...
  ...
  for(j = 0; j < nodes_y; j++)
  {
    for(i = 0; i < nodes_x; i++)
    {
      ...
      color_1->data[gid].x  = ofs + A*exp (
                                           -0.5*(
                                                 (position->data[gid].x + 0.0)*
                                                 (position->data[gid].x + 0.0) +
                                                 (position->data[gid].y + 0.2)*
                                                 (position->data[gid].y + 0.2))/sigma
                                          );                                                        // Setting "r" color coordinate...
      color_1->data[gid].y  = 0.0f;                                                                 // Setting "g" color coordinate...
      color_1->data[gid].z  = 0.0f;                                                                 // Setting "b" color coordinate...
      color_1->data[gid].w  = 1.0f;                                                                 // Setting "a" color coordinate...

      color_2->data[gid].x  = 0.0f;                                                                 // Setting "r" color coordinate...
      color_2->data[gid].y  = ofs + A*exp (
                                           -0.5*(
                                                 (position->data[gid].x + 0.2)*
                                                 (position->data[gid].x + 0.2) +
                                                 (position->data[gid].y - 0.2)*
                                                 (position->data[gid].y - 0.2))/sigma
                                          );                                                        // Setting "g" color coordinate...
      color_2->data[gid].z  = 0.0f;                                                                 // Setting "b" color coordinate...
      color_2->data[gid].w  = 1.0f;                                                                 // Setting "a" color coordinate...

      color_3->data[gid].y  = 0.0f;                                                                 // Setting "r" color coordinate...
      color_3->data[gid].z  = 0.0f;                                                                 // Setting "g" color coordinate...
      color_3->data[gid].z  = ofs + A*exp (
                                           -0.5*(
                                                 (position->data[gid].x - 0.2)*
                                                 (position->data[gid].x - 0.2) +
                                                 (position->data[gid].y - 0.2)*
                                                 (position->data[gid].y - 0.2))/sigma
                                          );                                                        // Setting "b" color coordinate...
      color_3->data[gid].w  = 1.0f;                                                                 // Setting "a" color coordinate...
  ...
  kernel_file.push_back (KERNEL_FILE);                                                              // Setting 1st source file...
  ...
  K->setarg (color_1, 1);                                                                           // Setting kernel argument "1"...
  K->setarg (color_2, 2);                                                                           // Setting kernel argument "2"...
  K->setarg (color_3, 3);                                                                           // Setting kernel argument "3"...
  K->setarg (t, 4);                                                                                 // Setting kernel argument "4"...

  ////////////////////////////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////// WRITING DATA ON OPENCL QUEUE //////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Q->write (position, 0);                                                                           // Uploading argument "0" on kernel...
  Q->write (color_1, 1);                                                                            // Uploading argument "1" on kernel...
  Q->write (color_2, 2);                                                                            // Uploading argument "2" on kernel...
  Q->write (color_3, 3);                                                                            // Uploading argument "3" on kernel...
  Q->write (t, 4);                                                                                  // Uploading argument "4" on kernel...

  ////////////////////////////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////// SETTING OPENGL SHADER ARGUMENTS ////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  S->setarg (position, 0);                                                                          // Setting shader argument "0"...
  S->setarg (color_1, 1);                                                                           // Setting shader argument "1"...
  S->setarg (color_2, 2);                                                                           // Setting shader argument "2"...
  S->setarg (color_3, 3);                                                                           // Setting shader argument "3"...
  ...
  while(!gui->closed ())                                                                            // Opening gui...
  {
    ...
    Q->write (t, 4);                                                                                // Writing time to OpenCL queue...

    Q->acquire (position, 0);                                                                       // Acquiring OpenGL/CL shared argument...
    Q->acquire (color_1, 1);                                                                        // Acquiring OpenGL/CL shared argument...
    Q->acquire (color_2, 2);                                                                        // Acquiring OpenGL/CL shared argument...
    Q->acquire (color_3, 3);                                                                        // Acquiring OpenGL/CL shared argument...
    ctx->execute (K, Q, NU_WAIT);                                                                   // Executing OpenCL kenrnel...
    Q->release (position, 0);                                                                       // Releasing OpenGL/CL shared argument...
    Q->release (color_1, 1);                                                                        // Releasing OpenGL/CL shared argument...
    Q->release (color_2, 2);                                                                        // Releasing OpenGL/CL shared argument...
    Q->release (color_3, 3);                                                                        // Releasing OpenGL/CL shared argument...
    ...
    Q->read (t, 4);                                                                                 // Reading time from OpenCL queue...
  ...
  delete color_1;                                                                                   // Deleting OpenGL color_1...
  delete color_2;                                                                                   // Deleting OpenGL color_2...
  delete color_3;                                                                                   // Deleting OpenGL color_3...
