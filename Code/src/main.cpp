...
#define KERNEL_FILE   "tile_kernel.cl"                                                              // OpenCL kernel.
...
#define SHADER_VERT   "tile.vert"                                                                   // OpenGL vertex shader.
#define SHADER_GEOM   "tile.geom"                                                                   // OpenGL geometry shader.
#define SHADER_FRAG   "tile.frag"                                                                   // OpenGL fragment shader.
...
int main ()
{
  ...
  float                    A                  = 1.0;                                                // Amplitude of gaussian.
  float                    sigma              = 0.2;                                                // Width of gaussian.
  ...
  color->name    = "tile_color";                                                                    // Setting variable name in OpenGL shader...
  ...
  for(j = 0; j < nodes_y; j++)
  {
    for(i = 0; i < nodes_x; i++)
    {
      ...
      color->data[gid].x    = A*exp (
                                     -0.5*(
                                           position->data[gid].x*position->data[gid].x +
                                           position->data[gid].y*position->data[gid].y)/sigma
                                    );                                                              // Setting "r" color coordinate...
      color->data[gid].y    = 0.0f;                                                                 // Setting "g" color coordinate...
      color->data[gid].z    = 0.0f;                                                                 // Setting "b" color coordinate...
