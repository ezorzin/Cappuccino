...
#define KERNEL_FILE   "cappuccino_kernel.cl"                                                        // OpenCL kernel.
...
int main ()
{
  ...
  float1G*                 height             = new float1G ();                                     // OpenGL float1G.
  float                    A                  = 50.0;                                               // Amplitude of gaussian.
  float                    sigma              = 0.3;                                                // Width of gaussian.
  ...
  height->init (nodes);                                                                             // Initializing OpenGL height array...
  ...
  height->name   = "voxel_height";                                                                  // Setting variable name in OpenGL shader...
  ...
  for(j = 0; j < nodes_y; j++)
  {
    for(i = 0; i < nodes_x; i++)
    {
      ...
      height->data[gid]     = A*exp (
                                     -0.5*(
                                           position->data[gid].x*position->data[gid].x +
                                           position->data[gid].y*position->data[gid].y)/sigma
                                    );                                                              // Setting "x" height level...
  ...
  K->setarg (height, 2);                                                                            // Setting kernel argument "2"...
  K->setarg (t, 3);                                                                                 // Setting kernel argument "3"...
  ...
  Q->write (height, 2);                                                                             // Uploading argument "2" on kernel...
  Q->write (t, 3);                                                                                  // Uploading argument "3" on kernel...
  ...
  S->setarg (height, 2);                                                                            // Setting shader argument "2"...
  ...
  while(!gui->closed ())                                                                            // Opening gui...
  {
    ...
    Q->release (height, 2);                                                                         // Releasing OpenGL/CL shared argument...
    ...
    Q->read (t, 3);                                                                                 // Reading time from OpenCL queue...
  ...
  delete height;                                                                                    // Deleting OpenGL height...
